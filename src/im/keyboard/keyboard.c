...
void* FcitxKeyboardCreate(FcitxInstance* instance)
{
    ...
    FcitxIsoCodes* isocodes = FcitxXkbReadIsoCodes(ISOCODES_ISO639_XML,
...
INPUT_RETURN_VALUE FcitxKeyboardDoInput(void *arg, FcitxKeySym sym, unsigned int state)
{
    ...
    if (IsDictAvailable(keyboard) && enableWordHint) {
        FcitxInputState *input = FcitxInstanceGetInputState(
            layout->owner->owner);

        if (FcitxHotkeyIsHotKey(sym, state,
                                layout->owner->config.hkAddToUserDict)) {
            FcitxModuleFunctionArg arg;
            arg.args[0] = keyboard->buffer;
            arg.args[1] = keyboard->dictLang;
            if (InvokeFunction(keyboard->owner, FCITX_SPELL,
                               ADD_PERSONAL, arg))
                return IRV_DO_NOTHING;
        }
        ...
        if (IsValidChar(result) || FcitxHotkeyIsHotKeySimple(sym, state) ||
            ...
            if (IsValidChar(result) || FcitxHotkeyIsHotKeyLAZ(sym, state) ||
                ...
                if (result)
                    ...
                    Ucs4ToUtf8(layout->owner->iconv,
                               FcitxKeySymToUnicode(sym), buf);
...
{
    FcitxConfigFileDesc *configDesc = GetKeyboardConfigDesc();
    FILE *fp = FcitxXDGGetFileUserWithPrefix("conf", "fcitx-keyboard.config", "w", NULL);
