...
fcitx_keyval_to_unicode (uint keyval)
{
    ...
    while (max >= min) {
        mid = (min + max) / 2;
        if (fcitx_keysym_to_unicode_tab[mid].keysym < keyval)
            min = mid + 1;
        else if (fcitx_keysym_to_unicode_tab[mid].keysym > keyval)
            max = mid - 1;
        else {
            /* found it */
            return fcitx_keysym_to_unicode_tab[mid].ucs;
        }
    }
...
fcitx_unicode_to_keyval (quint32 wc)
{
    ...
    while (max >= min) {
        mid = (min + max) / 2;
        if (fcitx_unicode_to_keysym_tab[mid].ucs < wc)
            min = mid + 1;
        else if (fcitx_unicode_to_keysym_tab[mid].ucs > wc)
            max = mid - 1;
        else {
            /* found it */
            return fcitx_unicode_to_keysym_tab[mid].keysym;
        }
    }
